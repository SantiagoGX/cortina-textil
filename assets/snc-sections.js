(() => {
    ...
    xhr.onload = function() {
      if (xhr.status >= 200 && xhr.status < 300) {
        try {
          ...
          const cartButtons = document.querySelectorAll('.custom-header--cart-button, .custom-header--icon-button[aria-label*="carrito"], .custom-header--icon-button[aria-label*="Carrito"]');
          ...
          cartButtons.forEach(function(btn) {
            ...
            if (itemCount > 0) {
              if (!countEl) {
                ...
                countEl.style.cssText = 'position:absolute;top:-5px;right:-5px;background:#2A4432;color:white;border-radius:50%;width:18px;height:18px;font-size:11px;display:flex;align-items:center;justify-content:center;font-weight:bold;';
      ...
      function showAddedFeedback(card) {
        try {
          ...
          if (button) {
            ...
            button.style.backgroundColor = '#2A4432';
          }
  ...
  function initProductListSection(section, tracker) {
    ...
    function initLoadMore(scope) {
      ...
      const setLoading = (isLoading) => {
        ...
        if (label) loadMoreBtn.textContent = isLoading ? 'Cargando...' : label;
      };
  ...
  api.register('snc-wishlist', (section) => {
    ...
    async function renderWishlist() {
      ...
      grid.innerHTML = '';
      grid.innerHTML = '<div class="snc-plp--loader"><div class="snc-plp--spinner"></div><p>Cargando tus favoritos...</p></div>';
